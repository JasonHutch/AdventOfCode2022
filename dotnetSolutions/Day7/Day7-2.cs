...
namespace Advent_Of_Code_2022.Day7
{
    class Day7Part2
    {
        ...
        List<int> bigEnoughToDelete = new List<int>();
        int totalNum = 0;
        ...
        public void run()
        {
            ...
            int targetDiskSpace = 30000000;
            int freeSpace = 0;
            int max = 0;
            int target = 0;
            int smallestFileToDelete = -1;
            ...
            freeSpace = totalDisk - max;
            target = targetDiskSpace - freeSpace;
            ...
            foreach (var item in totals) 
            {
                if(item > target)
                {
                    bigEnoughToDelete.Add(item); 
                }
            }

            foreach (var dirSize in bigEnoughToDelete)
            {
                if(smallestFileToDelete == -1)
                {
                    smallestFileToDelete = dirSize;
                }

                if (dirSize < smallestFileToDelete)
                {
                    smallestFileToDelete = dirSize;
                }
            }

            Console.WriteLine(smallestFileToDelete);
        }
